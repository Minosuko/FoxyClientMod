...
public class MonumentFinder extends Finder {
    ...
    public List<BlockPos> findInChunk() {
        ...
        // In Yarn 1.21.1, biome categories are less prominent, but we can still check name or other properties via features
        if (!Features.MONUMENT.isValidBiome(BiomeFixer.swap(biome))) return new ArrayList<>();
        ...
        result.forEach((pieceFinder, positions) -> {
            ...
            positions.forEach(pos -> {
                ...
                this.cuboids.add(new Cuboid(pos, pieceFinder.getLayout(), 0xFF0000FF)); // Blue
                this.cuboids.add(new Cuboid(monumentStart.getStartPos().add(0, pos.getY(), 0), 0xFF0000FF));
            });
        });
