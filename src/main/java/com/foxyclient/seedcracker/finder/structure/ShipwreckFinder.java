    ...
    private boolean onChestFound(BlockPos pos) {
        ...
        if (chestZ != 0) {
            ...
            if ((mutablePos.getX() & 0xf) == 0 && (mutablePos.getZ() & 0xf) == 0) {
                ...
                this.cuboids.add(new Cuboid(box, 0xFF00FFFF)); // Cyan
                this.cuboids.add(new Cuboid(new ChunkPos(mutablePos).getStartPos().offset(Direction.UP, mutablePos.getY()), 0xFF00FFFF));
                return true;
            }
        }
