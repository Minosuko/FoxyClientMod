...
import com.foxyclient.seedcracker.cracker.decorator.Dungeon;
...
import com.foxyclient.seedcracker.util.PosIterator;
...
public class DungeonFinder extends BlockFinder {
    ...
    public List<BlockPos> findInChunk() {
        ...
        if (result.size() != 1) return new ArrayList<>();
        Biome biome = this.world.getBiome(this.chunkPos.getBlockPos(8, 0, 8)).value();
        ...
        if (Config.get().getVersion().isNewerThan(MCVersion.v1_17_1)) {
            ...
            this.cuboids.add(new Cuboid(pos, 0xFFFF0000)); // Red
            FoxySeedCracker.get().getDataStorage().addBaseData(data, DataAddedEvent.POKE_BIOMES);
            return result;
        }
        ...
        if (AntiXRay(pos) && Config.get().antiXrayBypass) {
            if (FoxySeedCracker.get().getDataStorage().baseSeedData.contains(new DataStorage.Entry<>(data, null))) {
                return result;
            }
            this.cuboids.add(new Cuboid(pos, 0xFFFF0000)); // Red
            Thread floorCallsUpdater = new Thread(() -> {
                ...
                if (FoxySeedCracker.get().getDataStorage().addBaseData(data, data::onDataAdded)) {
                    if (data.usesFloor()) {
                        this.cuboids.add(new Cuboid(pos.subtract(size), pos.add(size.getX(), size.getY() - 1, size.getZ()), 0xFFFF0000));
                    }
                } else {
                    this.cuboids.clear();
                }
        ...
        } else if (FoxySeedCracker.get().getDataStorage().addBaseData(data, data::onDataAdded)) {
            this.cuboids.add(new Cuboid(pos, 0xFFFF0000)); // Red
    ...
    public void blockUpdateExploit(BlockPos pos, Vec3i size, Thread startCracker) {
        ...
        FoxySeedCracker.get().getDataStorage().blockUpdateQueue.add(floorBlocks, pos, startCracker);
    }
